...
tt_abd_ids = design_df.query("condition == 'treatment'")["gen_abd"].tolist()
...
def get_control_base_id(base_id: str) -> str:
    """Get control sample ID for given treatment sample."""
    cell_line = design_df.loc[design_df.base_id == base_id, "cell_line"].values[0]
    control_base_id = design_df.query('condition=="control" and cell_line==@cell_line')[
        "base_id"
    ].values[0]
...
diffbind_df["stringent_peak_files"] = (
    dynamic_unfiltered_seacr_dir
    / "stringent"
    / (diffbind_df["base_id"] + ".stringent.bed")
)
...
ctrl_abd_base_ids = (
    design_df.query("condition == 'control'")["base_id"].unique().tolist()
)
all_abd_base_ids = design_df["gen_abd"].unique().tolist()


def get_compute_matrix_args() -> str:
    """
    Generate command-line arguments for computeMatrix based on config file.
    """
    mode = config["deeptools"]["computeMatrix"]["mode"]
    reference_point = config["deeptools"]["computeMatrix"]["reference_point"]
    upstream = config["deeptools"]["computeMatrix"]["upstream"]
    downstream = config["deeptools"]["computeMatrix"]["downstream"]
    binsize = config["deeptools"]["computeMatrix"]["binsize"]
    average_type = config["deeptools"]["computeMatrix"]["averageType"]
    regionbody_length = config["deeptools"]["computeMatrix"]["regionbody_length"]
    region_file = config["deeptools"]["computeMatrix"]["regionsFileName"]
    use_whole_genome = config["deeptools"]["computeMatrix"]["use_whole_genome"]
    BED = str(REF_DIR / "genes.bed")

    # Base mode
    if mode == "scale-regions":
        args = f"scale-regions --regionBodyLength {regionbody_length}"
    elif mode == "reference-point":
        args = f"reference-point --referencePoint {reference_point}"
    else:
        raise ValueError(
            f"ERR: The mode set for deeptools computeMatrix '{mode}' is invalid."
        )

    # Add common arguments
    args += f" --upstream {upstream} --downstream {downstream} --binSize {binsize} --averageType {average_type}"

    # Evaluate whether the regions file is present or if whole genome is to be used
    if use_whole_genome:
        region_file_to_use = BED
    else:
        if region_file:
            region_file_to_use = region_file
        else:
            region_file_to_use = BED

    args += f" --regionsFileName {region_file_to_use}"

    return args
# for id in sorted(set(tt_abd_ids)):
#     print(get_control_id(id))
