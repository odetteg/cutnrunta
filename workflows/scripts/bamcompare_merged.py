...
gen_abd_id = snakemake.wildcards.gen_abd_id
...
mask = egs_df["sample"].str.contains(gen_abd_id)
...
if mask.any():
    ...
    with open(std_err, "a") as f:
        f.write(
            f"Warning: Could not correctly map {gen_abd_id} to its effective genome size. Using default: 2432564026\n"
        )
