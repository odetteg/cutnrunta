...
rule duplicates:
    ...
    log:
        std_err="logs/markdedup/{base_id}.mark_rmv_duplicates.err.log"
    params:
        rmv_duplicates=str(config["picard"]["rmv_duplicates"]).lower()
    shell:
        """
        module load picard/2.23.5
        picard MarkDuplicates -I {input.filtered_sorted_bam} -O {output.deduped} \
