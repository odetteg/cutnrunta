...
rule ac_lost:
    ...
    shell:
        """
        bedtools intersect -v -a {input.bl3_ac} -b {input.tbl3_ac} -v > {output.ac_lost} 2> {log}
        """
...
rule me_gained:
    ...
    shell:
        """
        bedtools intersect -v -a {input.tbl3_me} -b {input.bl3_me} -v > {output.me_gained} 2> {log}
        """
