...
if config["peaks_call"]["homer"]["use_homer"]:

    rule create_control_tag_dir:
        input:
            ctrl_bam=lambda wc: get_ctrl_bam(wc.ctr_id),
            ref_genome=config["REF_GENOME"],
        output:
            cttag_dir=directory(homer_dir / "control/{ctr_id}"),
        conda:
            "../../envs/homer.yaml"
        shell:
            """
            makeTagDirectory {output.cttag_dir} {input.ctrl_bam} -genome {input.ref_genome} -single
            """

    rule homer:
        input:
            tt_bam=mark_remove_dups / "{base_id}.deduped.bam",
            ctrl_tag_dir=lambda wc: homer_dir
            / "control"
            / get_control_base_id(wc.base_id),
            ref_genome=config["REF_GENOME"],
        output:
            peaks=homer_dir / "peaks/{base_id}_peaks.txt",
            ttag_dir=directory(homer_dir / "treatment/{base_id}"),
        params:
            style="histone",
        conda:
            "../../envs/homer.yaml"
        log:
            "logs/homer/{base_id}_homer.log",
        shell:
            """
            makeTagDirectory {output.ttag_dir} {input.tt_bam} -genome {input.ref_genome}  -single
            findPeaks {output.ttag_dir} -style {params.style} -i {input.ctrl_tag_dir} -size 1000 -L 0 -C 0 -nfr -o {output.peaks}
            """
